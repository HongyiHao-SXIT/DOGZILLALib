...
Interface for navigating to a certain location：ROS_UDP_Client_Start(str) #Among them, str is the point of position, with a total of 26 selectable points, 1-26 corresponding to A-Z，For example, navigate to point 1 or point A：ROS_UDP_Client_Start("APoint")
Description interface for navigation related images:Image_Contect_en(str)#Among them, str is the relevant on-site question asked，For example, describe what is inside the picture：Image_Contect_en("Describe what is inside the picture")
...
There are also some navigation related points: for example, the pantry is point A, the restroom is point B, and the conference room is point C.
...
My instructions: Navigate to the pantry to dance, then navigate to point C to do a push up, and finally navigate to point 2 to pee. You output:{"function":['ROS_UDP_Client_Start("APoint")','Dog_Dance()','ROS_UDP_Client_Start("CPoint")','Dog_push_up()','ROS_UDP_Client_Start("BPoint")','Dog_Pee()'],"response": "Navigation has started"}
My instruction: Help me go to the conference room to see if anyone is there, and then dance. You output:{"function":['ROS_UDP_Client_Start("CPoint")','Image_Contect_en('Is there anyone in the picture')','Dog_Dance()','response':'The testing conference room has started'}
