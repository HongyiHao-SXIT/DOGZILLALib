...
def on_message(ws, message):
    try:
        ...
        if code != 0:
            ...
            with open('./ros_dog_agent/DogAgent.mp3', 'ab') as f:
                f.write(audio)
                #print("iconm")
...
# 收到websocket错误的处理 Received processing for closing websocket
...
def on_open(ws):
    def run(*args):
        ...
        #print("------>开始发送文本数据")
        ws.send(d)
        if os.path.exists('./ros_dog_agent/DogAgent.mp3'):
            os.remove('./ros_dog_agent/DogAgent.mp3')

    thread.start_new_thread(run, ())


def Dog_speaktts(context="这个是合成音频程序"):
    global wsParam
    ...
    #播放音频 PLAY AUDIO
    proc = Popen("mplayer ./ros_dog_agent/DogAgent.mp3", shell=True) #sudo 
    proc.wait()
    time.sleep(0.5)
