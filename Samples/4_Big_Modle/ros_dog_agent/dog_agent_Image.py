...
导航到某个位置的接口：ROS_UDP_Client_Start(str) #其中str是位置的点,一共有26个可选的点位,1-26对应着A-Z，比如：导航到1号点位或者是A点：ROS_UDP_Client_Start("APoint")
导航相关的图片的描述接口:Image_Contect(str)#其中str是询问的相关现场的问题，比如:描述下图片里面有什么东西：Image_Contect("描述下图片里面有什么东西")
...
我的指令：导航到茶水间跳个舞，然后导航到C点做个俯卧撑，最后在导航到2号点位撒尿。你输出:{"function":['ROS_UDP_Client_Start("APoint")','Dog_Dance()','ROS_UDP_Client_Start("CPoint")','Dog_push_up()','ROS_UDP_Client_Start("BPoint")','Dog_Pee()'],"response": "导航开始了"}
我的指令：帮我去会议室看看有没有人，然后跳个舞。你输出:{"function":['ROS_UDP_Client_Start("CPoint")','Image_Contect('图片里有人没')','Dog_Dance()'],'response':'检测会议室开始了'}
