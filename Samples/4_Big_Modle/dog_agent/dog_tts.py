...
def on_message(ws, message):
    try:
        ...
        if code != 0:
            ...
            with open('./dog_agent/DogAgent.mp3', 'ab') as f:
                f.write(audio)
                #print("iconm")
...
def on_open(ws):
    def run(*args):
        ...
        #print("------>开始发送文本数据")
        ws.send(d)
        if os.path.exists('./dog_agent/DogAgent.mp3'):
            os.remove('./dog_agent/DogAgent.mp3')
...
def Dog_speaktts(context="这个是合成音频程序"):
    ...
    #播放音频 PLAY AUDIO
    proc = Popen("mplayer ./dog_agent/DogAgent.mp3", shell=True) #sudo 
    proc.wait()
    time.sleep(0.5)
