...
controller_interface::return_type JointTrajectoryController::update()
{
  ...
  const rclcpp::Duration & _period = node_->now() - prev_time;
      ...
      if (active_goal)
      {
        ...
        if (abort || outside_goal_tolerance)
        {
          ...
          if (abort)
          {
            ...
            set_error(result, FollowJTrajAction::Result::PATH_TOLERANCE_VIOLATED);
          }
          else if (outside_goal_tolerance)
          {
            RCLCPP_WARN(node_->get_logger(), "Aborted due to goal tolerance violation");
            set_error(result, FollowJTrajAction::Result::GOAL_TOLERANCE_VIOLATED);
          }
...
JointTrajectoryController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  if (use_closed_loop_pid_adapter)
  {
    ...
    for (size_t i = 0; i < pids_.size(); ++i)
    {
      ...
      RCLCPP_WARN_EXPRESSION(logger, k_p == 0.0,
                             "The 'p' value of joint '%s' is '0.0'. This joint won't move.",
  ...
  std::shared_ptr<trajectory_msgs::msg::JointTrajectory> trajectory_msg)
{
  ...
  auto remap = [this](
    ...
    {
      RCLCPP_WARN(node_->get_logger(), "Invalid input size (%zu) for sorting, expected: (%zu)",
...
void JointTrajectoryController::set_hold_position()
{
  ...
  resize_joint_trajectory_point(state_current, joint_names_.size());
  // current state update
  state_current.time_from_start.set__sec(0);
  read_state_from_hardware(state_current);
  ...
  trajectory_msgs::msg::JointTrajectory hold_msg;
  hold_msg.joint_names = joint_names_;
  hold_msg.header.stamp = rclcpp::Time(0);
  hold_msg.points = {state_current};

  auto traj_msg = std::make_shared<trajectory_msgs::msg::JointTrajectory>(hold_msg);
  add_new_trajectory_msg(traj_msg);
}
