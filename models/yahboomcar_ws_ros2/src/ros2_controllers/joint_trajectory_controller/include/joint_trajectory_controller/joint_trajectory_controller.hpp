...
protected:
  ...
  rclcpp::Time prev_time = rclcpp::Time(0, 0, (rcl_clock_type_t) RCL_ROS_TIME);
...
private:
  ...
  static void set_error(const std::shared_ptr<FollowJTrajAction::Result>& result, int32_t error_code)
  {
    result->set__error_code(error_code);
    std::string error_msg = std::to_string( error_code ) + " ";
    switch (error_code) {
      case FollowJTrajAction::Result::INVALID_GOAL:
        error_msg += "INVALID_GOAL";
        break;
      case FollowJTrajAction::Result::INVALID_JOINTS:
        error_msg += "INVALID_JOINTS";
        break;
      case FollowJTrajAction::Result::OLD_HEADER_TIMESTAMP:
        error_msg += "OLD_HEADER_TIMESTAMP";
        break;
      case FollowJTrajAction::Result::PATH_TOLERANCE_VIOLATED:
        error_msg += "PATH_TOLERANCE_VIOLATED";
        break;
      case FollowJTrajAction::Result::GOAL_TOLERANCE_VIOLATED:
        error_msg += "GOAL_TOLERANCE_VIOLATED";
        break;
      default: break;
    }
    result->set__error_string(error_msg);
  }
};
