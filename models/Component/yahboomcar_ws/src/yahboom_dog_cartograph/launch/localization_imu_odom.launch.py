...
def generate_launch_description():
    ...
    turtlebot3_cartographer_prefix = get_package_share_directory('yahboom_dog_cartographer')
  ...
  ## ***** File paths ******
    ...
    cartographer_node = Node(
        ...
        arguments = [
            '-configuration_directory',FindPackageShare('yahboom_dog_cartographer').find('yahboom_dog_cartographer') + '/config',
            '-configuration_basename', 'dog_2d.lua',
            '-load_state_filename', LaunchConfiguration('load_state_filename')],
        remappings = [
            ('imu', '/imu/data_raw_self'),
            ('scan', 'scan')],
        output = 'screen' 
        )
